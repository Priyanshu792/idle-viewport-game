...
func save_settings():
	...
	config.set_value("window", "custom_width", custom_size.x)
	config.set_value("window", "custom_height", custom_size.y)
...
func load_settings():
	...
	if config.load(SETTINGS_PATH) == OK:
		...
		var custom_width = config.get_value("window","custom_width",604)
		var custom_height = config.get_value("window","custom_height",604)
		custom_size = Vector2i(custom_width,custom_height)
...
func set_custom_resolution(custom_res_val_1,custom_res_val_2):
	...
	resolution_idx = RESOLUTIONS.find("CUSTOM")
